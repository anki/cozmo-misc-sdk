...
class TapCube(threading.Thread):
    ...
    async def set_up_cozmo(self, coz_conn):
        ...
        await self._robot.play_anim("anim_launch_wakeup_02").wait_for_completed()
        await self._robot.play_anim("anim_hiking_lookaround_01").wait_for_completed()
        await self._robot.play_anim("anim_hiking_lookaround_03").wait_for_completed()
        await self._robot.play_anim("anim_hiking_lookaround_02").wait_for_completed()
        await self._robot.set_head_angle(cozmo.util.degrees(0)).wait_for_completed()
    ...
    async def find_cube(self):
        ...
        try:
            ...
            return False
        if self._round < 2:
            tap_anim = random.choice(["anim_pyramid_reacttocube_happy_high_01", "anim_speedtap_winround_intensity02_02"])
            await self._robot.play_anim(tap_anim).wait_for_completed()
        if self._round > 2:
            return
        self._cube.start_light_chaser(GREEN)
        await self._robot.set_head_angle(cozmo.util.Angle(degrees=20.5)).wait_for_completed()
    ...
    async def go_to_cube(self,cube):
        ...
        await self._robot.go_to_object(cube, distance_mm(30.0)).wait_for_completed()
    ...
    async def see_fireworks(self,cube):
        if self._round != 2:
            await self._robot.drive_straight(distance_mm(-50), speed_mmps(80)).wait_for_completed()
            await self._robot.set_lift_height(0, duration=0.3).wait_for_completed()
        ...
        if self._animation_triggered is False:
            self._trigger_message = False
            await self._robot.play_anim("anim_keepaway_losehand_01").wait_for_completed()
        ...
        if self._animation_triggered and self._round < 2:
            ...
            # Cozmo acts excited
            if self._round == 0:
                await self._robot.set_head_angle(cozmo.util.Angle(degrees=44.5),1,1,1).wait_for_completed()
                await asyncio.sleep(1)
                await self._robot.play_anim("anim_reacttoblock_react_01_head_angle_40").wait_for_completed()
                await self._robot.drive_straight(distance_mm(-30), speed_mmps(30)).wait_for_completed()
                self._animation_triggered = False
                self._tap_action_finished = True

            # Dud
            if self._round == 1:
                await self._robot.play_anim("anim_reacttocliff_turtlerollfail_03").wait_for_completed()
                self._animation_triggered = False
                self._tap_action_finished = True
        # Grand finale
        if self._animation_triggered and self._round == 2:
            await asyncio.sleep(0.5)
            await self._robot.play_anim("anim_memorymatch_failhand_03").wait_for_completed()
            await asyncio.sleep(0.5)
            await self._robot.play_anim("reacttoblock_triestoreach_01").wait_for_completed()
            if self._animation_triggered:
                self.send_msg(4)
            if self._animation_triggered:
                self.send_msg(3)
            await self._robot.play_anim("anim_reacttoblock_react_01_head_angle_40").wait_for_completed()
            await self._robot.play_anim("reacttoblock_reacttotopple_01").wait_for_completed()
            await self._robot.play_anim("anim_reacttocliff_wheely_01").wait_for_completed()
    ...
    async def go_back_to_normal(self):
        self._cube.stop_light_chaser()
        self._cube.set_lights_off()
        await self._robot.play_anim("anim_guarddog_getout_untouched_01").wait_for_completed()
        await self._robot.play_anim("anim_guarddog_getout_timeout_01").wait_for_completed()
        await self._robot.play_anim("anim_gotosleep_getin_01").wait_for_completed()
